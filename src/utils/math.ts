...
const getRandomFloat = (min = 0.0, max = 1.0, fixed = 1) => {
  if (min > max) {
    return max;
  }
 ...
 * @returns 랜덤수
 */
export const getRandomNumber = (min: number, max: number, option?: RandomNumberOptionProps): number => {
  if (min > max) {
    return max;
  }
  if (option) {
    const { fixed } = option;
    if (fixed) {
      return getRandomFloat(min, max, fixed);
    }
  }
